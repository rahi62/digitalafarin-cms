...
const apiUrl = arg("--api-url", "http://localhost:8000/api/cms/v1");
...
const readme = `# DigitalAfarin CMS Admin app\n\nGenerated by @digitalafarin/cms-admin.\n\n## Development\n\n\`\`\`bash\nnpm install\nnpm run dev\n\`\`\`\n\nOpen: http://localhost:${port}${route}/\n\n## Production\n\n\`\`\`bash\nnpm run build\nPORT=${port} npm start\n\`\`\`\n\nAdd the contents of \`deploy/nginx.cms.conf\` to the HTTPS server block for your main domain.\n\nThe admin UI talks to the CMS backend through \`NEXT_PUBLIC_API_URL\`. For a separate API subdomain, configure Django CORS for the website origin. Alternatively proxy the API through the same website origin and set this value to that proxied path.\n`;
